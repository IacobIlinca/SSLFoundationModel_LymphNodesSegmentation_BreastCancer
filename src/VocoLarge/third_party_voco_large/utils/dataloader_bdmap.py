...
def get_loader_Atlas(args):
    ...
    """
    base_trans, random_trans = get_abdomen_trans(args)
...
def get_atlas_ds(args):
    ...
    data_dir = '/data/AbdomenAtlasMini1.0/'
    dataset_list = ['AbdomenAtlas1.0']
    data_txt_path = './jsons/dataset_list'
    cache_dir = '/data/cache/Atlas'

    for item in dataset_list:
        for line in open(os.path.join(data_txt_path, item + '.txt')):
            name = line.strip().split('\t')[0]
            train_img_path = os.path.join(data_dir, name, 'ct.nii.gz')
            label_img_path = os.path.join(data_dir, name, 'label.nii.gz')
    ...
    if args.cache:
        ...
        train_ds = Dataset(data=data_dicts_train, transform=transforms.Compose(base_trans))
